...
{
	assert(a1.ciphertext.compare(a2.plaintext) == 0 && "Cipher composition failed, text doesn't match.");
	AffineCipher result;
...
{
	assert(c1.ciphertext.compare(c2.plaintext) == 0 && "Cipher composition failed, text doesn't match.");
	CaesarCipher result;
...
{
	assert(v1.ciphertext.compare(v2.plaintext) == 0 && "Cipher composition failed, text doesn't match.");
